...
void
CVulkanCanvas::setGModule(CRenderer *renderer)
{
	mGModule = renderer;
}
...
void CVulkanCanvas::PickPhysicalDevice()
{
    if (!m_instance) {
        throw std::runtime_error("Programming Error:\n"
...
void CVulkanCanvas::CreateShaderModule(const std::vector<char>& code, VkShaderModule& shaderModule) const
{
...
{
	auto result = vkMapMemory(m_logicalDevice, memory, offset, size, 0, data);
...
void CVulkanCanvas::AllocateMemory(VkBuffer &buffer)
{
...
VkMemoryAllocateInfo CVulkanCanvas::CreateMemoryAllocateInfo(VkBuffer &buffer)
{
...
void CVulkanCanvas::CreateUniformBuffer(VkBuffer &buffer, uint32_t size)
{
