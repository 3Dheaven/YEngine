...
void
CVulkan::prepare(HWND *hwnd, const int& sizeW, const int& sizeH)
{
	mSizeW = sizeW;
	mSizeH = sizeH;
	...
	m_swapChain.CreateSwapChain(sizeW, sizeH);
...
void CVulkan::CreateCommandBuffers()
{
	...
	{
		vkFreeCommandBuffers(logicalDevice, m_device.mCommandPool, (uint32_t)m_commandBuffers.size(), m_commandBuffers.data());
	}
...
void CVulkan::recreateSwapchain()
{
	...
	m_swapChain.CreateSwapChain(mSizeW, mSizeH);
...
void CVulkan::CreateVertexBuffer(VkBuffer &buffer, VkDeviceMemory &deviceMemorie)
{
	...
	m_device.CreateBuffer(stagingBuffer, VK_BUFFER_USAGE_TRANSFER_SRC_BIT, (uint32_t)bufferSize,
	...
	m_device.CreateBuffer(buffer, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_VERTEX_BUFFER_BIT, (uint32_t)bufferSize,
...
void CVulkan::CreateIndexBuffer(VkBuffer &buffer, VkDeviceMemory &deviceMemorie)
{
	...
	m_device.CreateBuffer(buffer, VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT, (uint32_t)bufferSize,
		VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, deviceMemorie);
...
CVulkan::setupDescriptorSet()
{
	...
	for (auto &u : mUniforms)
	{
		...
		descriptorWrite.dstBinding = (uint32_t)mWriteDescriptorSets.size();
