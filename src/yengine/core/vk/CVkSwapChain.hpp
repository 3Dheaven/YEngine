...
required to support other windowing systems.
	...
	void CreateSwapChain(const int& sizeW, const int& sizeH)
	{
		...
		VkExtent2D extent = ChooseSwapExtent(swapChainSupport.capabilities, sizeW, sizeH);
	...
	VkExtent2D ChooseSwapExtent(const VkSurfaceCapabilitiesKHR& capabilities,
		const int& sizeW, const int& sizeH) const noexcept
	{
		...
		{
			VkExtent2D actualExtent = { static_cast<uint32_t>(sizeW),
				static_cast<uint32_t>(sizeH) };
